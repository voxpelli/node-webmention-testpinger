...
app.get('/template', function (req, res) {
  ...
  fs.readFile(__dirname + '/../templates/' + name + '.html', {encoding: 'utf8'}, function (err, data) {
    if (err) {
      ...
      console.log(chalk.grey('At ' + (new Date()).toTimeString()), chalk.blue.bold(name), 'was fetched.');
  ...
  templates.forEach(function (file) {
    ...
    request({
      ...
      if (!error && response.statusCode > 199 && response.statusCode < 300) {
        console.log(chalk.grey('At ' + (new Date()).toTimeString()), chalk.green.bold(file), 'pinged.');
      } else {
        console.error(chalk.grey('At ' + (new Date()).toTimeString()), chalk.red.bold(file), 'failed to be pinged.');
